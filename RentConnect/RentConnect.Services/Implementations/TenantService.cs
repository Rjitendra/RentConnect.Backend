    ...
    public class TenantService : ITenantService
{
    ...
    public async Task<Result<TenantSaveResponseDto>> CreateTenants(TenantCreateRequestDto request)
    {
        ...
        try
        {
            ...
            // Create AspNetUsers records for all tenants
            try
            {
                var createdUserIds = await CreateTenantUsers(createdTenants);

                // Log successful user creations
                foreach (var userCreation in createdUserIds)
                {
                    Console.WriteLine($"Created AspNetUser for tenant {userCreation.Key} with ID: {userCreation.Value}");
                }
            }
            catch (Exception ex)
            {
                // Log error but don't fail the entire tenant creation
                Console.WriteLine($"Warning: Failed to create some AspNetUsers records: {ex.Message}");
            }
    ...
    public async Task<Result<TenantSaveResponseDto>> UpdateTenant(TenantCreateRequestDto request)
    {
        ...
        try
        {
            ...
            var groupValidationErrors = ValidateTenantGroup(request.Tenants, request.IsSingleTenant.Value);
...
<body>
    ...
    private async Task<Dictionary<string, long>> CreateTenantUsers(List<TenantDto> tenants)
    {
        ...
        foreach (var tenant in tenants)
        {
            try
            {
                ...
                if (userId > 0)
                {
                    createdUsers[tenant.Email] = userId;
                }
                else if (userId == -1)
                {
    ...
    private ApplicationUserDto MapTenantToApplicationUser(TenantDto tenant)
    {
        ...
        {
            Email = tenant.Email,
            UserName = tenant.Email, // Using email as username
            FirstName = firstName,
            LastName = lastName,
            PhoneNumber = tenant.PhoneNumber ?? "",
        ...
        foreach (var tenant in tenants)
        {
            try
            {
                ...
                if (existingUser != null)
                {
                    ...
                    if (userId > 0)
                    {
                        updatedUsers[tenant.Email] = userId;
                    }
                }
            }
            catch (Exception ex)
